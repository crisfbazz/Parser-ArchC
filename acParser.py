...
class acParser(object):
  ...
  __ac_cache_parm = 'ac_cache_parm'  + __lThan + delimitedList(__symbol ^ __number) + __gThan + __semiColon
  __ac_cache_parm.setParseAction(actions.AddNewItem)
  ...
  __ac_port2 = 'ac_tlm2_port' + delimitedList(__ac_item) + __semiColon
  __ac_port2.setParseAction(actions.AddSubItems)
  __ac_port2_nb = 'ac_tlm2_nb_port' + delimitedList(__ac_item) + __semiColon
  __ac_port2_nb.setParseAction(actions.AddSubItems)
  __ac_intr_port = "ac_tlm_intr_port" + delimitedList(__ac_item) + __semiColon
  __ac_intr_port.setParseAction(actions.AddSubItems)
  __ac_intr_port2 = "ac_tlm2_intr_port" + delimitedList(__ac_item) + __semiColon
  __ac_intr_port2.setParseAction(actions.AddSubItems)

  # New ac_asm comment declarations
  __asm_com_dec = "assembler" + __dot + (
    Literal("set_comment") | "set_line_comment" ) + __lPar + __str + __rPar + __semiColon

  __map_list = __lBrack + __number + __dot * (2, 2) + __number + __rBrack
  __map_body = ((delimitedList(__str) + __equal + __number) ^ (
    __str + __map_list + Optional(__str) + __equal + __map_list) ^ \
  \
  ...
  __reglist = __comma + delimitedList(__symbol + Optional((__equal | __plus) + ( ( 'reg' + __dot + 'map_to' + __lPar + __str + __rPar) | ( __symbol | __number | __str ) ) + Optional(OneOrMore(__plus+__symbol)) ))
  ...
  __opt_instr = __symbol + __dot + oneOf(__opt_list)  + __lPar+ SkipTo(OneOrMore(__rPar + __semiColon),include=True)
  ...
  __storagedecs = __ac_cache | __ac_mem | __ac_regbank | __ac_reg | __ac_port | __ac_intr_port | __ac_port | __ac_port2 | __ac_port2_nb | __ac_intr_port | __ac_intr_port2
  __ac_arch_commands = ZeroOrMore(
    __ac_cache_parm | __storagedecs | __ac_wordsize | __ac_isa | __set_endian | __ac_fetchsize | __ac_stage | \
    __ac_pipe | __ac_format | __bindTo )
  __ac_isa_commands = ZeroOrMore(
    __ac_format | __set_asm | __set_decoder | __ac_instr | __pseudo_instr | __ac_asm_map | \
  ...
  def ParseFile(self, fileName):
    ...
    """
    try:
      print "ArchC: Parsing AC_ARCH declaration file: " + fileName
      ...
      if 'ac_isa' in self.info:
        ...
        print "ArchC: Parsing AC_ISA declaration file: " + self.info['ac_isa']
        self.__AC_ISA.parseFile(self.info['ac_isa'])
    except ParseException, err:
      print "Error"
      print err.msg
      print err
      print "ArchC ERROR: Parser terminated unsuccessfully."
